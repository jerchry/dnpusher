...
public class CameraHelper implements SurfaceHolder.Callback, Camera.PreviewCallback {
    ...
    private void startPreview() {
        try {
            ...
            // 数据缓存区，是一种提高性能的方式
            // 给Camera添加一个缓冲区，系统会将采集到的数据填写到缓冲区里，
            // 然后我们通过设置回调接口：setPreviewCallbackWithBuffer(PreviewCallback previewCallback)，
            // 在onPreviewFrame(data, camera)中获得NV21数据，就是一张图片
    ...
    /**
     * 设置摄像头 图像传感器的角度、方向
     * 预览是正的，而data数据依然是倒的：mPreviewCallback.onPreviewFrame(bytes, camera);
     * 参考：android.hardware.Camera中setDisplayOrientation()的注释
     * @param parameters
     */
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        ...
        // 预览的是正的，而data数据依然是倒的，所以需要在前面处理data数据
        // 将数据回调给调用者
